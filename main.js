...
function init()
{
    ...
    img.onload = function ()
    {
        ...
        loadModel('trash/', 'grade.obj', 'grade.mtl', 'bush');
    };
...
function loading()
{
    green_sprt_tree.sprt = null;
    green_sprt_tree.tex = textureLoader.load("sprites/green.png", ld);
    
    green_sprt_house.sprt = null;
    green_sprt_house.tex = textureLoader.load("sprites/green.png", ld);

    green_sprt_bush.sprt = null;
    green_sprt_bush.tex = textureLoader.load("sprites/green.png", ld);
...
function ld()
{
    ...
    if (lscnt == 3)
    {
        green_sprt_tree.sprt = ls(green_sprt_tree.tex, new THREE.Vector3(-window.innerWidth / 2 + 94, window.innerHeight / 2 - 170, 0));
        green_sprt_house.sprt = ls(green_sprt_house.tex, new THREE.Vector3(-window.innerWidth / 2 + 200, window.innerHeight / 2 - 170, 0));
        green_sprt_bush.sprt = ls(green_sprt_bush.tex, new THREE.Vector3(-window.innerWidth / 2 + 374, window.innerHeight / 2 - 170, 0));
    }
}

var parasha;

function ls(texture, pos)
{
    var material = new THREE.SpriteMaterial({map: texture});
    var width = material.map.image.width;// window.innerWidth;//material.map.image.width;
    var height = material.map.image.height;//window.innerHeight;//material.map.image.height;
    var sprite = new THREE.Sprite(material);
    parasha = width / 100;
    sprite.scale.set(width/100, height / 25, 1);
...
function createParticle(pos)
{
    ...
    t.a = 3*Math.PI/2;
    t.b = -3*Math.PI/2;
    t.speed = (Math.random()*0.2) + 0.1;
    t.otklonienie = Math.PI/((Math.random()*3)+3);
    t.dir = 1; // +-1
...
function update()
{
    ...
    {
        {
            if ((particles.length < 1000)&&(check))
            {
                var x = Math.random() * N;
                var y =  70;
                var z = Math.random() * N;
        ...
        for(var i = 0; i < particles.length; i++)
        {
            ...
            //=================================================================
            t.x = particles[i].speed*Math.sin(particles[i].a)*Math.cos(particles[i].b);
            t.y = particles[i].speed*Math.sin(particles[i].a)*Math.sin(particles[i].b);
            t.z = particles[i].speed*Math.cos(particles[i].a);
            
            particles[i].a += particles[i].otklonienie * delta * particles[i].dir;
            particles[i].b += particles[i].otklonienie * delta * particles[i].dir;
            
            if (particles[i].a > 7*Math.PI/4)
            {
                particles[i].dir = -1;
            }
            
            if (particles[i].a < 5*Math.PI/4)
            {
                particles[i].dir = 1;
            }
            //=================================================================
            ...
            if (delay > 0.1)
            {
                ...
                p.sprt = smokeparticle.sprt.clone();

                p.sprt.position.copy(particles[i].sprt.position);
...
//                delay = 0.0;
            ...
            if ((particles[i].sprt.position.y <= 0)&&(check ))
            {
                 particles[i].sprt.position.copy(new THREE.Vector3(x, 150, z));
...
function createGUI()
{
    ...
    meshRY.onChange(function (value)
    {
        if (models.length > 0)
        {
            ...
            m2.setPosition(picked.userData.obb.position);
            var m1 = new THREE.Matrix4();
            
            m1.makeScale(picked.userData.obb.halfSize.x, picked.userData.obb.halfSize.y, picked.userData.obb.halfSize.z);
            //m1.multiplyScalar(2);
            var m = new THREE.Matrix4();
            //console.log(m1);
            //m.multiply(picked.userData.cube.scale);
            /*!!!!!!!!!!!!!!!!!!!!!*/m.multiplyMatrices(m2, picked.userData.obb.basis);//!!!!!!!!!!!!!!!
            //m.multiplyMatrices(picked.userData.obb.bas);
            var m4 = new THREE.Matrix4();
            var m5 = new THREE.Matrix4();
            m4.multiplyMatrices(m,m1);
            //m4.multiplyMatrices(m, m4);
           
            //m[10] = picked.userData.obb.basis[10];
            console.log(m4);
            picked.userData.cube.matrix = m4;
            picked.userData.cube.matrixAutoUpdate = false;
    ...
    var cubeVisible = gui.add( params, 'ch' ).name('ch').listen();
...
function addMesh(i, s)
{
    ...
    var material = new THREE.MeshBasicMaterial( 
     { color: 0xE0FFFF, wireframe: true }
    );
    
    models[models.length - 1].userData.cube = new THREE.Mesh( geometry, material );
    models[models.length - 1].userData.cube.position.copy(obb.position);
    
    models[models.length - 1].userData.cube.scale.copy(obb.halfSize.multiplyScalar(2));
    
    //m2.multiplyMatrices(setPosition(obb.position), obb.basis);
    
    
    scene.add( models[models.length - 1].userData.cube );
...
{    
    if (name === 'tree')
    {
        if (proc !== 100)
        {
            green_sprt_tree.sprt.scale.x = parasha * (proc);
        }
        else
        {
            green_sprt_tree.sprt.scale.set(0,0,0);
        }
    }
    if (name === 'house')
    {
        if (proc !== 100)
        {
            green_sprt_house.sprt.scale.x = parasha * (proc);
        }
        else
        {
            green_sprt_house.sprt.scale.set(0,0,0);
        }
    }
    if (name === 'bush')
    {
        if (proc !== 100)
        {
            green_sprt_bush.sprt.scale.x = parasha * (proc);
        }
        else
        {
            green_sprt_bush.sprt.scale.set(0,0,0);
        }
    }
}
        ...
        if (pressed)
        {
            ...
            if (picked != null)
                if (intersects.length > 0)
                {
                    ...
                    picked.userData.cube.position.copy(intersects[ 0 ].point);
                    picked.userData.bbox.update();
                    picked.userData.bbox.box.center(picked.userData.obb.position);
                    var m2 = new THREE.Matrix4();
            m2.setPosition(picked.userData.obb.position);
...
function onDocumentMouseDown(event)
{
    ...
    } else
    {
        ...
        if (intersects.length > 0)
        {
            ...
            picked.userData.bbox.material.visible = false;
            console.log(picked.userData.cube.position);
            picked.userData.cube.position.copy(intersects[0].object.userData);
        } else
        {
            picked = null;
        }
